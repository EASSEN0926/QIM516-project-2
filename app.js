...
function playTTS(text) {
  if (!("speechSynthesis" in window)) {
    alert("Your browser does not support text-to-speech. You can add local audio files in an audio folder.");
    return;
  }
...
function playPhrase(phrase) {
  ...
  const audio = new Audio(encodeURI(exactSrc));
  currentPhraseAudio = audio;

  audio.onerror = () => {
    console.warn("Audio file could not be played, using Malay TTS fallback:", exactSrc);
    playTTS(phrase.ms);
  };

  audio.play().catch(() => {
    console.warn("Browser blocked or failed audio playback, using Malay TTS fallback:", exactSrc);
    playTTS(phrase.ms);
  });
}
  ...
  root.innerHTML = `
    ...
    <section class="section" id="phrases">
      <div class="section-head">
        ...
        <p>Click Listen to hear the phrase. The page can play your local audio files, and it also has a Malay text-to-speech fallback.</p>
      </div>
