...
function playPhrase(phrase) {
  // Compatible audio loading:
  // The phrase list keeps the original .mp3 path, but this function will try:
  // 1) audio/[phrase-id].mp3
  // 2) audio/[phrase-id].m4a
  // 3) audio/[phrase-id].mp4
  // If none exists or none can play, it falls back to browser Malay TTS.
  const baseSrc = phrase.audio ? phrase.audio.replace(/\.[^/.]+$/, "") : "";
  if (!baseSrc) {
    playTTS(phrase.ms);
    return;
  }
  ...
  function tryNext() {
    ...
    // Some browsers do not always fire canplaythrough quickly.
    audio.load();
    setTimeout(() => {
      if (audio.readyState >= 2 && !failed) {
        audio.play().catch(failOnce);
      } else if (audio.readyState === 0 && !failed) {
        failOnce();
      }
    }, 700);
  }
