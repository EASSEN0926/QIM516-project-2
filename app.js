...
function playPhrase(rawPhrase) {
  ...
  if (!exactSrc) {
    console.error("No matching audio file for phrase:", phraseId, phrase.ms || rawPhrase);
    alert("Audio file not found for this phrase.");
    return;
  }

  const audio = new Audio(exactSrc);
  currentPhraseAudio = audio;

  audio.onerror = () => {
    console.error("Audio file exists in map but could not be loaded or played:", exactSrc);
    alert("Audio cannot be loaded. Please check the audio file on GitHub.");
  };

  audio.play().catch((error) => {
    console.error("Browser failed to play audio:", exactSrc, error);
    alert("Audio playback failed. Try clicking the button again or check the file format.");
  });
}
